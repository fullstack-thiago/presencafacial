...
export default function App() {
  ...
  const registerVideoRef = useRef(null); // video element used in register screen
  const mainVideoRef = useRef(null);     // video element used in fullscreen camera
  ...
  const DETECTION_INTERVAL_MS = 700; // ms
  ...
  useEffect(() => {
    if ((route === 'register' || route === 'attendance' || cameraFullscreen) ) {
      const t = setTimeout(() => {
        openCamera();
      }, 300);
  ...
  useEffect(() => {
    ...
    if (autoRecognitionEnabled) {
      if (mainVideoRef.current && streamRef.current && recognitionEnabled) {
  ...
  // Batch flusher for pending inserts
  useEffect(() => {
    const t = setInterval(async () => {
      const pending = pendingInsertsRef.current.splice(0, 200);
      if (!pending || pending.length === 0) return;
      try {
        const { error } = await supabase.from('attendances').insert(pending);
        if (error) {
          console.error('Erro batch insert attendances', error);
          // requeue
          pendingInsertsRef.current.unshift(...pending);
        } else {
          // fetch latest attendances lightly
          fetchAttendances({ company_id: selectedCompany });
        }
      } catch (err) {
        console.error('Erro no batch flusher', err);
        pendingInsertsRef.current.unshift(...pending);
      }
    }, 6000);
  ...
  async function fetchEmployees(companyId) {
    ...
    const normalized = (data || []).map((emp) => {
      ...
      if (typeof descs === 'string') {
        try { descs = JSON.parse(descs); } catch (err) { descs = []; }
      }
  ...
  async function fetchRegisteredEmployees() {
    try {
      ...
      if (selectedCompany) q = q.eq('company_id', String(selectedCompany));
  ...
  async function warmUpVideoFrames(frames = 6, msBetween = 80, videoEl) {
    if (!videoEl) return;
    const v = videoEl;
  ...
  async function checkCameraPermission() {
    try {
      ...
      try { const p = await navigator.permissions.query({ name: 'camera' }); return p.state; } catch (err) { return 'unknown'; }
  ...
  async function openCamera(opts = { skipAutoStartRecognition: false, retrying: false }) {
    ...
    try {
      ...
      if (preferredDeviceId) {
        ...
        constraints = { video: { facingMode, width: { ideal: 640 }, height: { ideal: 480 } } };
      }

      setStatusMsg('Pedindo permissão para câmera...');
      const stream = await navigator.mediaDevices.getUserMedia(constraints);

      const activeVideo = cameraFullscreen ? mainVideoRef.current : registerVideoRef.current;
      if (!activeVideo) {
      ...
      await activeVideo.play().catch((e) => { console.warn('play() falhou:', e); });
      ...
      await new Promise((resolve) => {
        const checkReady = () => {
          const v = activeVideo;
          if (v && v.videoWidth > 0 && v.videoHeight > 0) resolve();
      ...
      setTimeout(() => {
        ...
        if (canvas && v) {
          ...
          canvas.width = Math.round((v.videoWidth || v.clientWidth) * ratio);
          canvas.height = Math.round((v.videoHeight || v.clientHeight) * ratio);
      ...
      await warmUpVideoFrames(6, 60, activeVideo);
  ...
  async function switchFacing() {
    ...
    try {
      ...
      if (streamRef.current) {
        try { streamRef.current.getTracks().forEach((t) => t.stop()); } catch (e) { console.error('Erro ao parar stream:', e); }
  ...
  function stopCamera() {
    if (streamRef.current) {
      try { streamRef.current.getTracks().forEach((t) => t.stop()); } catch (e) { console.error('Erro ao parar tracks', e); }
      streamRef.current = null;
    }
    const activeVideo = cameraFullscreen ? mainVideoRef.current : registerVideoRef.current;
    if (activeVideo) activeVideo.srcObject = null;
  ...
  async function captureDescriptorFromVideo(opts = { attempts: 3, inputSize: 128, scoreThreshold: 0.45 }) {
    if (!faceapiLoaded || !faceapi) { setStatusMsg('Modelos não carregados ainda'); return null; }
    const videoEl = registerVideoRef.current;
    if (!videoEl) { setStatusMsg('Vídeo não disponível'); return null; }
  ...
  async function handleCaptureForRegister() {
    ...
    const desc = await captureDescriptorFromVideo({ attempts: 4, inputSize: 160, scoreThreshold: 0.45 });
    if (!desc) {
    ...
    for (const payload of payloads) {
      try {
        if (isUpdate && updateId) {
          ...
          if (error) { console.warn('update attempt error for payload:', payload, error); continue; }
          return { success: true, data };
        } else {
          const { data, error } = await supabase.from('employees').insert([payload]).select();
          if (error) { console.warn('insert attempt error for payload:', payload, error); continue; }
          return { success: true, data };
        }
      } catch (err) { console.error('insert/update exception for payload:', payload, err); continue; }
    }
  ...
  async function saveNewEmployee() {
    ...
    if (capturedDescriptors.length === 0 && !editEmployeeId) { alert('Capte ao menos 1 foto (ou edite sem novas capturas)'); return; }
  ...
  async function prepareFaceMatcherAndStart() {
    if (!selectedCompany) { setStatusMsg('Selecione uma empresa'); return; }
    if (!faceapiLoaded || !faceapi) { setStatusMsg('Modelos não carregados'); return; }
    ...
    if (e) { console.error(e); setStatusMsg('Erro buscando funcionários'); return; }
    if (!emps?.length) { setStatusMsg('Nenhum funcionário cadastrado'); return; }
  ...
  function startRecognitionLoop() {
    ...
    const loop = async (timestamp) => {
      ...
      const minInterval = (window.innerWidth <= 540 ? DETECTION_INTERVAL_MS - 200 : DETECTION_INTERVAL_MS);
      if (timestamp - lastRun < minInterval) return;
      lastRun = timestamp;
      ...
      try {
        ...
        if (Date.now() - lastDetectionsAtRef.current > 7000) {
          // relax parameters if no detection for a while
        }

        const optionsFast = new faceapi.TinyFaceDetectorOptions({ inputSize, scoreThreshold });
        const detections = await faceapi
          .detectAllFaces(activeVideo, optionsFast)
          .withFaceLandmarks()
          .withFaceDescriptors();
        ...
        lastDetectionsAtRef.current = Date.now();
        ...
        for (const det of detections) {
          ...
          if (label !== 'unknown') {
            ...
            // enqueue insert instead of querying DB for last record (reduces reads)
            pendingInsertsRef.current.push({ company_id: String(selectedCompany), employee_id: matchedEmployeeId, confidence: best.distance, attended_at: new Date().toISOString() });
  ...
  function exportAttendancesToExcel() {
    ...
    const rows = attendances.map((r) => ({ id: r.id, employee: r.employees?.name || r.employee_id, department: r.employees?.department || '', company: r.companies?.name || r.company_id, attended_at: r.attended_at, confidence: r.confidence }));
  ...
  function applyHistoryFilter(range) {
    ...
    if (range === 'all') { fetchAttendances({ company_id: selectedCompany }); }
    else { fetchAttendances({ company_id: selectedCompany, gte_attended_at: from }); }
  }

  function handleFilterChange(e) { const val = e.target.value; applyHistoryFilter(val); }

  // ---------- Registered employees actions ----------
  function handleSearchChange(e) { const v = e.target.value; setSearchName(v); }

  const filteredRegisteredEmployees = registeredEmployees.filter((emp) => { if (!searchName) return true; return (emp.name || '').toLowerCase().includes(searchName.toLowerCase()); });
  ...
  async function handleDeleteEmployee(empId) {
    ...
    try {
      ...
      if (error) { console.error('Erro ao apagar funcionário', error); alert('Erro ao apagar funcionário'); return; }
      setStatusMsg('Funcionário apagado');
      fetchEmployees(selectedCompany);
      fetchRegisteredEmployees();
      fetchAttendances({ company_id: selectedCompany });
    } catch (err) { console.error('Erro handleDeleteEmployee', err); alert('Erro ao apagar funcionário'); }
  }
  ...
  function handleCancelEdit() { setEditEmployeeId(null); setNewName(''); setNewDepartment(''); setCapturedDescriptors([]); }
                  ...
                  <div className="row gap">
                    <div className="col">
                      ...
                      <div style={{ marginTop: '15px', display: 'flex', gap: '10px' }}>
                        <button className="btn primary" onClick={() => { if (!selectedCompany) { alert('Selecione uma empresa antes de registrar presença'); return; } setCameraFullscreen(true); openCamera(); }}>
                          Abrir Câmera
                        </button>
...
<div className="card">
  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
    ...
    <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
      <input className="input" placeholder="Buscar por nome..." value={searchName} onChange={handleSearchChange} style={{ width: 200, minWidth: 120 }} />
      <button className="" onClick={() => { fetchRegisteredEmployees(); fetchEmployees(selectedCompany); setStatusMsg('Lista atualizada'); }} title="Atualizar lista">🔄</button>
    </div>
  </div>
  ...
  <div className="table-wrap" style={{ marginTop: 12 }}>
    <table className="table responsive-table">
      ...
      <tbody>
        {filteredRegisteredEmployees.length === 0 && (<tr><td colSpan={5} style={{ padding: 20 }} className="muted">Nenhum funcionário encontrado</td></tr>)}
    ...
    <div className="employee-cards">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginLeft: 12 }}>
              ...
              {route === 'register' && (
                <div className="card">
                  ...
                  {/* camera preview for register */}
                  <div className="video-wrapper">
                    <video ref={registerVideoRef} className="video" autoPlay muted playsInline />
                ...
                <div className="card">
                  ...
                  <div className="filter-row" style={{ display:'flex', alignItems:'center', gap:'8px', margin:'12px 0' }}>
                    ...
                    <select id="filterSelect" style={{ padding:'6px 10px', borderRadius:'6px' }} onChange={handleFilterChange}>
                      ...
                      <option value="day">Até 24 horas</option>
                      <option value="week">Até 7 dias</option>
                      <option value="month">Até 30 dias</option>
      ...
      {/* Fullscreen camera view overlay */}
      {user && cameraFullscreen && (
        <div className="camera-fullscreen">
          <video ref={mainVideoRef} className="video-fullscreen" autoPlay muted playsInline />
          ...
          <div className="recent-matches left">
            ...
            <ul>
              {recentMatches.map((m) => (<li key={m.id + String(m.timestamp)}>{m.name}</li>))}
            </ul>
          </div>

          <div className="camera-controls centered">
            <button className="btn-switch-camera glass" onClick={() => { switchFacing(); stopRecognitionLoop(); setTimeout(() => openCamera(), 400); }}>🔄</button>

            <button className="btn-switch-camera glass" onClick={() => { recognitionEnabled ? handleStopRecognition() : handleStartRecognition(); }} title={recognitionEnabled ? 'Parar reconhecimento' : 'Iniciar reconhecimento'} style={{ marginLeft: 10 }}>{recognitionEnabled ? '⏸️' : '▶️'}</button>

            <button className="btn-switch-camera glass" onClick={() => handleRestartCamera()} title="Reiniciar câmera" style={{ marginLeft: 10 }}>🎥</button>
          </div>
        </div>
      )}
